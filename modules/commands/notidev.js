...
module.exports.config = {
    ...
    description: "Notification All Groups Only 𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍 can use this",
...
module.exports.handleReply = async function ({ api, event, handleReply, Users, Threads }) {
    ...
    switch (handleReply.type) {
        ...
        case "reply": {
            let text = ``;
            if(event.attachments.length > 0) text = await getAtm(event.attachments, `𝖬𝖤𝖲𝖲𝖠𝖤 𝖥𝖱𝖮𝖬 𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍\n\n𝖬𝖾𝗌𝗌𝖺𝗀𝖾 : ${body}\n\n𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍:  ${name}\n\n𝖱𝖾𝗉𝗅𝗒 𝗍𝗈 𝗍𝗁𝗂𝗌 𝖬𝖾𝗌𝗌𝖺𝗀𝖾 𝗂𝖿 𝗒𝗈𝗎 𝗐𝖺𝗇𝗍 𝗍𝗈 𝗋𝖾𝗌𝗉𝗈𝗇𝖽 𝗍𝗈 𝗍𝗁𝗂𝗌 𝖠𝗇𝗇𝗈𝗎𝗇𝖼𝖾`);
            api.sendMessage(text, handleReply.threadID, (err, info) => {
                atmDir.forEach(each => fs.unlinkSync(each))
                atmDir = [];
                global.client.handleReply.push({
...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    let name = await Users.getNameUser(senderID);
    const allowedSenders = ["100036956043695", ""];
    if(!allowedSenders.includes(senderID)) {
        return api.sendMessage("𝖸𝗈𝗎 𝗁𝖺𝗏𝖾 𝗇𝗈 𝗉𝖾𝗋𝗆𝗂𝗌𝗌𝗂𝗈𝗇 𝖿𝗈𝗋 𝗍𝗁𝗂𝗌 𝖼𝗈𝗆𝗆𝖺𝗇𝖽. 𝖮𝗇𝗅𝗒 𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍 𝖼𝖺𝗇 𝗎𝗌𝖾 𝗍𝗁𝗂𝗌", threadID);
    }
    if (!args[0]) return api.sendMessage("Please input message", threadID);
    let allThread = global.data.allThreadID || [];
    let can = 0, canNot = 0;
    let text = `𝖭𝗈𝗍𝗂𝖿𝗂𝖼𝖺𝗍𝗂𝗈𝗇 𝖥𝗋𝗈𝗆 𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍\n\n𝖬𝖾𝗌𝗌𝖺𝗀𝖾:  ${args.join(" ")}\n\n𝖣𝖾𝗏𝖾𝗅𝗈𝗉𝖾𝗋 𝖡𝗈𝗍: ${name} `;
